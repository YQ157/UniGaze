...
def main():
	...
	with zipfile.ZipFile(zip_file_path, 'r') as zip_file:
		...
		image_files = [f for f in zip_contents if f.endswith('.png') and not f.endswith('_seg.png')]
		image_files = sorted(image_files)
		landmark_files = [ f.split('.')[0] + '_ldmks.txt' for f in image_files]
		
		img_pool = []
		for i in track(range(0, len(image_files)) ):
			img_path = image_files[i]
			ldmk_path = landmark_files[i]
...
if __name__ == '__main__':
	...
	save_path = os.path.join(output_dir, 'FaceSynthetics_224.h5')

...
def main():
	...
	sample_dir = osp.join(output_dir, 'sample'); os.makedirs(sample_dir, exist_ok=True)

	group_list = sorted(glob.glob(os.path.join(input_dir, 'group*.zip')) )
	group_list = [ os.path.basename(group) for group in group_list]

	print('group_list: ', group_list)
	for group in group_list[:]:
		g = group.split('.')[0]

		zip_file_path = os.path.join(input_dir, group)

		part_name = input_dir.split('/')[-1]
		save_path = osp.join(output_dir, f'{part_name}.h5')

		with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
			# Create a dictionary to hold files by their directories (clips)
			clips = defaultdict(list)
			
			# Loop through each file in the ZIP
			for file_name in zip_ref.namelist():
				# Skip directories (these names will end with '/')
				if file_name.endswith('/'):
					continue
				
				# Extract the directory (clip) and file name
				clip_name = file_name.split('/')[0]
				# Group the files by their clip
				clips[clip_name].append(file_name)
			
			# Now, loop through each clip
			for clip_name, file_list in clips.items():
				print(f"Processing {clip_name}: {len(file_list)} files")



				img_pool = []
				for i in track( range(0, len(file_list), img_sample_rate) ):
					img_path = file_list[i]

					print(f"  - Reading {img_path}")
					with zip_ref.open(img_path) as file:
						# Read the image file (as binary, for example, to process further)
					...
					if len(img_pool) == 25:
						...
						cv2.imwrite(osp.join(sample_dir, f'sample_{clip_name}_{i+1}.jpg'), grid)
						img_pool = []

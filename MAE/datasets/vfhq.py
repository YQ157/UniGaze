...
class VFHQDataset(Dataset):
	def __init__(self, 
				...
				imgs_person_use:int=None, ## number of images per person to use
		...
		self.imgs_person_use = imgs_person_use
	...
	def build_idx_to_kv(self):
		...
		for num_i in range(0, len(self.selected_keys)):
			...
			if self.limit_head_pose is not None:
				...
				if self.imgs_person_use is not None:
					indices = np.random.choice(indices, self.imgs_person_use, replace=False)
			else:
				if self.imgs_person_use is not None:
					n = min(n, self.imgs_person_use)
				indices = range(n)
